...
import { createClausificationError, createEngineError, createError, LogicException } from './types/errors.js';
...
export function clausesToProlog(clauses: Clause[]): string[] {
    ...
    for (const clause of clauses) {
        ...
        if (positive.length === 0) {
            ...
            throw createEngineError('Cannot convert non-Horn clause to Prolog. Try using the SAT engine.');
        }
    }
